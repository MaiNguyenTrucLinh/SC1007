...
int HashInsert(int key, HashSlot hashTable[])
{
   ...
   int hash = hash1(key);
    ...
    hashTable[insert].indicator = USED;
    return j;
}
...
int HashDelete(int key, HashSlot hashTable[])
{
    ...
    while (hashTable[hashVal].indicator != EMPTY && i < TABLESIZE)
    {
        if (hashTable[hashVal].indicator == USED && hashTable[hashVal].key == key)
        {
            hashTable[hashVal].indicator = DELETED;
            return i + 1;
        }
        hashVal += stepSize;
        hashVal %= TABLESIZE;
